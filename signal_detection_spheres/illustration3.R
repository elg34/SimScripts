...
verghese_mod2<-function(sig,n_targ,n_dist,sim, t_type = FALSE){
  ...
  for (i in 1:round(sim/2)){ 
    ...
    sign_rel<-if (n_dist==0){max(rnorm(n_dist+n_targ,mu1,sd1))}else if (t_type==TRUE){max(c(rnorm(7,mu2,sd2),rnorm(1,mu2^2,sd2)))}else{max(c(rnorm(7,mu1,sd1),rnorm(1,mu2,sd2)))}
...
p <- ggplot(df, aes(x=dv, fill=s)) + geom_density(alpha=.3) + facet_wrap(~ c) + ggtitle(paste("Underlying Signal (T1/D7): ",sig)) + theme(text = element_text(size=20),plot.title = element_text(hjust = 0.5, face="bold"))
